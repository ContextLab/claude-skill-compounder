...
|CI|`.github/workflows/ci.yml` runs the suite on ubuntu and macos, `shellcheck` on both, and `claude plugin validate --strict`. All five jobs passed on run `33977345612` (2026-09-05), against `4c0e9ae`, the tree that carries the lesson gate's final shape and the re-recorded screencast. Read the current one rather than this line: `gh run list --repo ContextLab/claude-skill-compounder --limit 1`|
