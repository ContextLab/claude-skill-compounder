...
**The default forge is A, C and D — two dispatched agents and two rounds.** The orchestrator
(B) and the judge (E) are dispatched only on a forge whose round budget has been raised past
2, and their briefs live under *Only past two rounds*, below. Every rule above that heading
applies to every forge.
...
stage A had to catch it at step 5. Neither cold reviewer could have. **A stranger cannot audit
...
4. **The acceptance test.** The original triggering problem written out as a task to
   attempt again, with the finished skill, at step 5.
5. **The framing check, written here rather than audited later.** Put the verbatim trigger
   and A's framing adjacent, and add one sentence saying what the framing generalises and
   what it drops. This is E's first question, asked at the moment the answer is still cheap:
   a misframing has every later check certifying the wrong thing, and on a two-round forge
   there is no E to catch it afterwards. Across ten forges the judge that used to ask it
   produced two meta findings and never a skill defect, which is why the question moved here
   and the stage did not survive at the default budget.
6. **The level, and what is deferred to `CLAUDE.md`.** General > user > project: a skill
   belongs at the highest level of the hierarchy it applies to, and the specialisation comes
   from the `CLAUDE.md` that already records it rather than from text baked into the skill.
   Record how routing will be verified at the chosen level too, since where a skill installs
   decides which router sees it. This was B's first decision; on a two-round forge there is
   no B, and a decision nobody is assigned is a decision nobody makes.
...
nothing. See the standing quiesce rule below.

**Returns at step 5:** the result of running the finished skill against the real case, and
...
## Standing rules, on every forge

These three hold whoever is driving the loop — the session on a two-round forge, the orchestrator
on one that was escalated past it.

**Standing rule: nothing reads the draft while the builder is still writing it.** Before anyone
...
- C writes a marker file as the literal last action of a round, e.g. `printf 'round 3 done\n'
  > <scratch>/ROUND-DONE`, and whoever dispatched it waits for that file to appear;
- then read the draft's checksum twice, a few seconds apart, and proceeds only if the two
...
dispatcher were observed looping back to the sender instead of arriving, and the loop
...
**Standing rule: a fix is confirmed by running it**, never by trusting the report that
claims it. This is not suspicion for its own sake: builders in this loop have reported fixes that
...
**Holds:** the dispatcher's brief, the authoring standard, and a scratch working directory. **No path into
...
to write the marker its dispatcher is watching for; the standing rules above say why a message
is not enough and why nothing reads the draft until that file exists and the checksum has
settled.

**Must build a runnable reproduction where the skill has an executable surface, and only
there.** If the draft documents a command, a script, a file format or an API call, C
constructs in its scratch directory the smallest thing that exhibits the situation the brief
described — a repo with two commits, a failing test, a file with the wrong permissions — and
runs every command the draft documents against it. That is what makes isolation compatible
with the rule that unverified claims are defects: C cannot check the original case, so it
builds a case it can check.

**Where there is no executable surface there is nothing to reproduce.** A procedure whose
steps are judgements about prose, or about what to do next, buys a scratch repo nobody reads.
No note records the mandatory repro catching a defect on such a skill, and demanding one on
every forge is a cost paid every time for a benefit that arrives sometimes.

**C states which of the two it is, in one sentence, in the round record.** That sentence is
the whole guard: without it "no executable surface" becomes the default answer, said by the
agent with the most to gain from saying it.

**A repro that is owed and cannot be built is the finding.** It means the brief was abstract
enough to be unexecutable, which is a defect in the brief. It is never a reason for C to
proceed on description alone, and never a reason to go looking for the real project.
...
## Only past two rounds

Nothing below this line is dispatched on a default 2-round forge. `skillforge escalate` is
what brings these two stages back, and the round it grants is the round they run in.
...
- that those six prompts are **run** at step 5 against a real router, over at least three
...
- **The cap.** 2 rounds for a straightforward skill, 4 for one that is complex or
  safety-critical. Fixed at `start` because `skillforge` encodes it in `<total-steps>`, and
  the only way past it is `skillforge escalate`, one round at a time, twice at most.
...
**Holds:** the finished skill, A's brief file, A's step-5 verification, and — handed
...
Sections appended in order by A, C and D — and by B and E on a forge that ran past two rounds
and had them. Each signs its own: who it was, what it was given, what it concluded. **Nobody edits anyone else's section.** Where two sections
...
## Step 4: feeding findings back without stalling the loop
