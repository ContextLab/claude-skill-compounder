...
#
# 7. COST. jq over the whole 26 MB transcript, extracting every tool output, took 0.22 s
#    against a 10 s hook timeout. The session-wide scan is affordable, which is why the
#    PostToolUse accumulator is optional rather than required. A 663 MB transcript has been
#    measured in the wild, hence MAX_BYTES.
#
# ====================================================================================
...
#
# STOP ARM: 205 real closing messages -> 6 blocks (2.9%).
#   All 6 are Tier 1; Tier 2 fired zero times on this corpus.
...
#   CLAIM_GATE_MAX_BYTES (67108864) transcript read budget; larger files are tail-read.
...
MAX_BYTES="${CLAIM_GATE_MAX_BYTES:-67108864}"
...
if [ "$event" = "Stop" ]; then
  ...
  {
    sed -nE "s/.*(^|[[:space:]])(-m|--message)[=[:space:]]+\"([^\"]*)\".*/\3/p" "$TMP/cmd.txt"
    sed -nE "s/.*(^|[[:space:]])(-m|--message)[=[:space:]]+'([^']*)'.*/\3/p" "$TMP/cmd.txt"
    awk '/<<-?[A-Za-z_'"'"'"]/{inh=1; next} inh{print}' "$TMP/cmd.txt"
...
if [ "$TIER2" != "0" ]; then
  claim_re='all[[:space:]]+(the[[:space:]]+)?tests?[[:space:]]+(still[[:space:]]+)?(pass|passing|passed|green)|tests?[[:space:]]+(all[[:space:]]+)?pass(es|ing|ed)?([^A-Za-z]|$)|(test[[:space:]]+)?suite[[:space:]]+(is[[:space:]]+)?(green|passes|passing|clean)|full[[:space:]]+suite[[:space:]]+(is[[:space:]]+)?(green|passes|passing)|everything[[:space:]]+passes|[0-9,]+[[:space:]]*(/|[[:space:]]+of[[:space:]]+)[[:space:]]*[0-9,]+[[:space:]]+(tests?[[:space:]]+)?(green|passing|pass)|[0-9,]+[[:space:]]+tests?[[:space:]]+(pass|passing|passed|green)|ci[[:space:]]+(is[[:space:]]+)?green|all[[:space:]]+green|build[[:space:]]+(is[[:space:]]+)?(green|clean|succeeds|succeeded)'

  runner_re='run_tests|pytest|py\.test|unittest|npm[[:space:]]+(run[[:space:]]+)?test|yarn[[:space:]]+test|pnpm[[:space:]]+test|go[[:space:]]+test|cargo[[:space:]]+test|make[[:space:]]+(test|check)|ctest|jest|mocha|vitest|rspec|tox|bats|phpunit|dotnet[[:space:]]+test|gradle[[:space:]]+test|mvn[[:space:]]+test|gh[[:space:]]+run|gh[[:space:]]+pr[[:space:]]+checks|gh[[:space:]]+workflow|plugin[[:space:]]+validate'
  ...
  if normalise_claim_text "$TMP/msg.txt" | grep -qiE "$claim_re" 2>/dev/null; then
    ...
    jq -rR --arg runner "$runner_re" --arg skip "$skip_path_re" 'fromjson? // empty
      | select((has("isSidechain")|not) or (.isSidechain != true))
      | if .type=="assistant" then
          ...
          ((.toolUseResult
           ...
           | if test("Ran [0-9]+ tests?|[0-9]+ (passed|failed)|OK \\(|FAILED \\(|=+ [0-9]+ (passed|failed)|ALL TESTS|\"conclusion\": *\"(success|failure)\"")
             then "@@RUN@@" else empty end)
        else empty end
    ' "$TMP/scan.jsonl" > "$TMP/marks.txt" 2>/dev/null || : > "$TMP/marks.txt"
    [ -f "$TMP/marks.txt" ] || : > "$TMP/marks.txt"

    last_run="$(grep -n '^@@RUN@@$' "$TMP/marks.txt" 2>/dev/null | tail -1 | cut -d: -f1)"
    last_mut="$(grep -n '^@@MUT@@$' "$TMP/marks.txt" 2>/dev/null | tail -1 | cut -d: -f1)"
    [ -z "$last_run" ] && last_run=0
    [ -z "$last_mut" ] && last_mut=0

    if [ "$last_run" -eq 0 ]; then
      add_finding "a test suite is claimed to pass, but nothing in this session ran a test command -- no tool call matched a known test or CI runner and no tool output carried a runner summary line."
